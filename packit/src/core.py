...
class PackItCore:
    ...
    def uninstallPlugin(self, pluginId, autoRestart=False):
        def task():
            try:
                from android.content import Context
                from org.telegram.messenger import ApplicationLoader
                ...
                pluginFile = File(f"/data/data/{packageName}/files/plugins/{pluginId}.py")
                pycacheFile = File(f"/data/data/{packageName}/files/plugins/__pycache__/{pluginId}.cpython-311.pyc")
                ...
                if not pluginFile.exists():
                    ...
                    run_on_ui_thread(showError)
                    log(f"plugin file '{pluginId}.py' not found")
                    return
                ...
                if pycacheFile.exists():
                    ...
                    if pycacheDeleted:
                        log(f"pycache file '{pluginId}.cpython-311.pyc' deleted")
                    else:
                        log(f"failed to delete pycache file '{pluginId}.cpython-311.pyc'")
                ...
                if pluginDeleted:
                    ...
                    run_on_ui_thread(showSuccess)
                    log(f"plugin '{pluginId}.py' uninstalled successfully")
                    ...
                    run_on_ui_thread(showError)
                    log(f"failed to delete plugin file '{pluginId}.py'")
                ...
                run_on_ui_thread(showError)
                log(f"failed to uninstall plugin '{pluginId}': {e}")
                import traceback
                log(f"traceback: {traceback.format_exc()}")
