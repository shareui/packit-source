...
class _DocumentHandler(MethodHook):
    ...
    def _prepare_and_show(self, file_path: str):
        try:
            from .bin.Writer import _get_user_id, _get_install_ts
            from .bin.Reader import read_file
            ...
            if "achievements" in blocks:
                try:
                    from ....ui.achievements.service.AchivementsEngine import get_level_info
                    ...
                    if isinstance(achievements_data, dict) and achievements_data and all(_is_hashed_id(k) for k in achievements_data):
                        from ....ui.achievements.service.AchivementsEngine import _hash_account_id
    ...
    def _restore(self, blocks: dict, account_id: str):
        try:
            from .bin.Reader import _write_blocks
            from client_utils import get_last_fragment
