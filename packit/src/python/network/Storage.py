...
# Everything the plugin fetches from a repository over the network: its repomap,
# its plugin list, its icon list, its avatar.
#
# Every screen that wanted a plugin list used to GET it itself and unpack a
# "plugins" value that is an object in some repositories and an array in
# others. Nine copies of that existed, no two identical: the User-Agent was
# sent by one of them, the timeout for the same file ranged from 10 to 20
# seconds, and only one of the three places that fetched a repomap had the full
# table of http reasons the add dialog localises. A repository is asked once,
# from here.
#
